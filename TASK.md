...
# 17. MANGWON REAL DATA DEMO — FEASIBILITY GATE RECORD
...
`GATE_DATE = 2026-09-13 (Asia/Seoul)`

`SOURCES = 망원시장 공식 점포 안내, Google Maps 장소 상세/좌표, Google Maps Street View 브라우저 probe, 기존 web 코드·테스트`
...
- 공식 점포 상세: `https://www.mangwonmarket.com/stores-restaurant-kr/?bmode=view&idx=167464653`(훈훈호떡), `idx=167464646`(맛있는집), `idx=167464639`(부산대원어묵), `idx=167464636`(우이락).
- Google Maps는 5개 점포 모두 장소 상세와 좌표를 반환했다. Place ID는 영구 식별자로 저장하지 않고 현재 검증용 근거로만 사용한다.
- Street View browser probe는 점포 좌표에서 한 번 수행했다. Google Maps 웹 UI가 nearest panorama의 정확한 미터를 노출하지 않아 표에는 `좌표 probe / exact distance not exposed`를 적용한다. 구현에서는 기존 `50m → 30m` 좌표 기반 재조회 규칙을 유지한다.
- Google Maps JavaScript API 키와 billing/domain restriction은 코드로 생성·승인할 수 없으므로 `EXTERNAL_ACTION_REQUIRED: Google Maps API key/billing/domain restriction`이다. 키가 없어도 schema·실제 점포 데이터·상세 UI·navigation 연결은 계속 구현한다.
- 현장 출입구, 실제 도보 도착, GNSS 오차, Street View와 눈앞 점포의 시각 일치는 `FIELD_VALIDATION_PENDING`이다.

## Gate conclusion

핵심 구조(실제 점포 선택 → 실제 좌표 → 현재 위치 → 기존 K-Navi routing)는 구현 가능하고, 일부 점포의 Google Street View 품질/공식 목록 표기 차이만 남아 있으므로 `PARTIAL GO`로 판정한다. 명세에 따라 Store schema·검증 데이터·망원시장 구간 UI·상세/Street View 연결을 계속 구현한다.
