...
# Phase A. 경로 안내 정보 고도화 (ETA·총거리·안내문)
...
# Phase C. 목적지 검색을 TMAP POI 통합검색으로 전환
...
1. **대중교통 연계**: TMAP 대중교통 API로 "도보+지하철/버스+도보" 복합 경로.
   Free 요금제에 포함되는지, 별도 신청인지 콘솔에서 먼저 확인한다.
...
| POI 통합검색 | `GET /tmap/pois?version=1` | C | 미착수 |
| 역지오코딩 | `GET /tmap/geo/reversegeocoding?version=1` | C | 미착수 |
| Static Map | `GET /tmap/staticMap` | D | 미착수 |
| 대중교통 | (콘솔에서 상품/한도 확인) | E | 보류 |
