...
class TestGpsPollNeeded:
    ...
    def test_regression_typing_before_first_fix_pauses_poll(self):
        # ★버그 재현 방지★ 화면 뜨자마자(첫 fix 미취득) 목적지 입력 중이면 폴링을 멈춰야
        # 한다 — 예전엔 origin 없을 때 폴링이 계속 돌아 st_searchbox 입력이 리셋됐다.
    ...
    def test_typing_pauses_even_with_booking_armed(self):
        # autorefresh 게이팅과 동일 — 입력 중이면 예약 폴링도 멈춘다.
        assert self._call(dest_entry_active=True, booking_armed=True, origin_present=False) is False

    def test_running_always_polls_even_if_flagged_dest_entry(self):
        # 안내 중엔 입력 상태가 성립하지 않지만, 방어적으로 running 이 우선한다.
        assert self._call(running=True, dest_entry_active=True) is True
