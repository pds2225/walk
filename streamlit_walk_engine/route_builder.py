...
def _tmap_addr_results(query: str, limit: int = 5) -> list[tuple[Coordinate, str]]:
    ...
    POI(건물·장소) 검색과 별개인 주소 전용 API. 건물·상호로 등록되지 않은 순수
    지번·도로명 주소지도 잡으므로, Naver 지오코딩이 키 없음·결과 없음일 때
    TMAP 앱키 하나로 주소 검색을 대신한다 — 배포 환경에 Naver 키가 없으면
    검색이 건물(POI) 중심으로만 돌아 주소지가 안 뜨던 문제의 근본 보강.
    '서판로30'처럼 번호가 붙은 표기는 공백 변형('서판로 30')도 순서대로 시도.
    앱키 없음·오류·결과 없음이면 빈 리스트(다음 폴백으로 넘어감).
    """
    app_key = _tmap_app_key()
    if not app_key:
        return []
    for q in _road_number_variants(query):
        try:
            resp = requests.get(
                _TMAP_ADDR_GEO,
                params={"version": "1", "fullAddr": q, "coordType": "WGS84GEO"},
                headers={"appKey": app_key, "Accept": "application/json"},
                timeout=_TIMEOUT,
            )
            if resp.status_code != 200:
                continue
            rows = (((resp.json() or {}).get("coordinateInfo") or {})
                    .get("coordinate") or [])
        except (requests.RequestException, KeyError, ValueError,
        ...
        for row in rows[:limit]:
            if not isinstance(row, dict):
                continue
            # 도로명 매칭은 new* 필드(newLat/newLon·newRoadName), 지번 매칭은
            # lat/lon·legalDong/bunji 에 담긴다. 표시 주소는 _tmap_reverse 와 같은
            # 원칙 — 도로명은 roadName 이 실제로 있을 때만 조립(없으면 지번 폴백이
            # city_do·gu_gun 만으로 죽지 않게), 읍·면·리는 있으면 끼워 시골 주소 보존.
            coord = None
            for lat_key, lon_key in (("newLat", "newLon"), ("lat", "lon")):
                try:
            ...
            if coord is None:
                continue
            road = ""
            road_name = (row.get("newRoadName") or row.get("roadName") or "").strip()
            if road_name:
                road = " ".join(p for p in (
                    row.get("city_do"), row.get("gu_gun"), row.get("eup_myun"),
                    road_name,
                    row.get("newBuildingIndex") or row.get("buildingIndex"),
                ) if p)
                building = (row.get("newBuildingName")
                            or row.get("buildingName") or "").strip()
                if road and building:
                    road = f"{road} {building}"
            jibun = " ".join(p for p in (
                row.get("city_do"), row.get("gu_gun"), row.get("eup_myun"),
                row.get("legalDong") or row.get("adminDong"), row.get("ri"),
                row.get("bunji"),
            ) if p)
            display = (road or jibun).strip()
            if not display:
                continue
            out.append((coord, display))
        if out:
...
def geocode_suggestions(query: str, limit: int = 5,
    ...
    주소 후보(Naver → 키 없음·결과 없음이면 TMAP fullAddrGeo)를 먼저 채우고,
    남은 자리는 TMAP 장소(POI) 후보로 항상 보충한다 — 주소지와 건물·장소가
    함께 뜨게. 둘 다 비면 Nominatim 변형 검색 폴백.
