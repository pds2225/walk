...
# 저장 내용(좌표·라벨)이 그대로여도 ts 는 주기적으로 갱신해 장시간 안내가 만료되지 않게 한다.
_ACTIVE_SESSION_REFRESH_MS = 5 * 60 * 1000
...
def _save_active_session() -> None:
    ...
    if running and dest is not None:
        ...
        # ts 원값 대신 갱신 버킷만 비교해 매 rerun 쓰기는 피하면서 만료 기준은 계속 민다.
        sig_obj = {k: obj[k] for k in ("lat", "lon", "label", "transit")}
        sig_obj["ts_bucket"] = obj["ts"] // _ACTIVE_SESSION_REFRESH_MS
        sig = json.dumps(sig_obj, ensure_ascii=False)
...
def _resume_has_new_destination(resume: dict) -> bool:
    """자동 재개 대기 중 사용자가 새 목적지를 입력/선택했는지 확인한다."""
    saved_label = str(resume.get("label") or "").strip()
    dest_text = str(st.session_state.get("nav_dest_input") or "").strip()
    if dest_text and dest_text != saved_label:
        return True
    if st.session_state.get("nav_dest_picked") is not None:
        return True
    sb = st.session_state.get("nav_dest_sb")
    if isinstance(sb, dict):
        search = str(sb.get("search") or "").strip()
        if search and search != saved_label:
            return True
        if sb.get("result") is not None:
            return True
    return False
...
def main() -> None:
    ...
    if resume is not None:
        ...
        action = nav_session.resume_action(
            ...
            has_new_destination=_resume_has_new_destination(resume),
