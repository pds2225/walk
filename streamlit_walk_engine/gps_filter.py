...
# ── 재탐색 결과 전달 채널(페이지 밖 영속 보관) ───────────────────────────────
# Streamlit 페이지 스크립트는 rerun 마다 처음부터 다시 실행돼 '페이지 모듈 전역'이
# 매번 초기화된다(백그라운드 워커가 결과를 넣어도 다음 rerun 은 새 dict 를 봄 — E2E 로
# 실증된 함정). 반면 import 된 이 모듈은 sys.modules 에 캐시되어 프로세스 동안 유지되므로
# 'rerun 을 건너 살아남아야 하는' 재탐색 결과는 여기 둔다. 키=세션 id.
PENDING_REROUTE: dict = {}
PENDING_REROUTE_LOCK = threading.Lock()
...
# ── 진행 방향(heading) 보정 ─────────────────────────────────────────────────
# 보행 중 GPS heading/course 는 저속·지터로 매 표본 수십도씩 튄다(화살표·헤딩업 지도가
# 떨림). 최근 heading 들을 '원형 평균'으로 합쳐(각도는 359°↔1° 가 인접하므로 산술평균은
# 틀림 — sin/cos 벡터합으로) 진행 방향을 안정화한다. 최근일수록 큰 가중치를 줘 실제 회전에
# 대한 반응은 유지하면서 지터만 눌러 준다.
HEADING_SMOOTH_WINDOW = 5  # 원형 평균에 쓰는 최근 heading 표본 수


def smooth_heading(
    headings,
    window: int = HEADING_SMOOTH_WINDOW,
) -> Optional[float]:
    """최근 heading 목록의 '가중 원형 평균'(진북 시계방향 0~360)을 돌려준다.

    - None 항목은 무시한다(측정 결측). 유효값이 없으면 None.
    - 최근 window 개만 사용하고, 뒤(최근)일수록 가중치를 선형으로 키운다 —
      떨림은 누르되 실제 회전에는 빠르게 따라오도록.
    - 각도는 원형(wrap-around)이라 벡터(sin,cos) 합으로 평균한다. 산술평균은
      350°와 10° 를 180°(정반대) 로 잘못 섞는다.
    반환은 [0,360) 로 정규화. 순수 함수 — 세션·부수효과 없음.
    """
    vals = [float(h) for h in (headings or []) if h is not None]
    if not vals:
        return None
    vals = vals[-window:]
    n = len(vals)
    ...
    for i, h in enumerate(vals):
        w = float(i + 1)  # 1..n : 최근일수록 큰 가중치
        rad = math.radians(h)
        sx += w * math.cos(rad)
        sy += w * math.sin(rad)
    if sx == 0.0 and sy == 0.0:
        # 정확히 상쇄되는 드문 경우(정반대 방향 동일가중) — 가장 최근값으로 폴백.
        return vals[-1] % 360.0
